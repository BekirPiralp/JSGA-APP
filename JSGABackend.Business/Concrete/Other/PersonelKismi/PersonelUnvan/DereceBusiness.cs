using JSGABackend.Business.Abstract.Other.PersonelKismi.PersonelUnvan;
using JSGABackend.Business.Concrete.Base;
using JSGABackend.DataAccess.Abstract.Other.PersonelKismi.PersonelUnvan;
using JSGABackend.Entity.Concrete.Other.PersonelKismi.PersonelUnvan;

namespace JSGABackend.Business.Concrete.Other.PersonelKismi.PersonelUnvan
{
    public class DereceBusiness : BaseForBusiness<IDereceDataAccess, Derece>, IDereceBusiness
    {

    }
}
